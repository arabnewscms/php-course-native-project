	...
	<form method="post" action="<?php echo aurl('users/edit?id='.$user['id'] ); ?>" enctype="multipart/form-data">
		...
		<div class="row">
			<div class="col-md-6">
				<div class="form-group">
					...
					<input type="text" name="name" placeholder="<?php echo trans('user.name'); ?>"
						class="form-control <?php echo  !empty(get_error('name'))?'is-invalid':'' ; ?>" value="<?php echo  $user['name'] ; ?>" />
				</div>
			</div>
			...
			<div class="col-md-6">
				<div class="form-group">
					...
					<input type="text" name="email" placeholder="<?php echo trans('user.email'); ?>"
						class="form-control <?php echo  !empty(get_error('email'))?'is-invalid':'' ; ?>" value="<?php echo  $user['email'] ; ?>" />
				</div>
			</div>
			...
			<div class="col-md-6">
				<div class="form-group">
					...
					<input type="text" name="mobile" placeholder="<?php echo trans('user.mobile'); ?>"
						class="form-control <?php echo  !empty(get_error('mobile'))?'is-invalid':'' ; ?>" value="<?php echo  $user['mobile'] ; ?>" />
				</div>
			</div>
          ...
          <select class="form-select <?php echo  !empty(get_error('user_type'))?'is-invalid':'' ; ?>" name="user_type">
            <option value="user" <?php echo  $user['user_type'] == 'user'?'selected':'' ; ?>><?php echo trans('user.user'); ?></option>
            <option value="admin" <?php echo  $user['user_type'] == 'admin'?'selected':'' ; ?>><?php echo trans('user.admin'); ?></option>
          </select>
